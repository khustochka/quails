...
Quails.pages.cards = {
  ...
  initFastSpeciesLinks: function () {
    ...
    return $('.fast-sp-link').click(function () {
      ...
      $('[data-taxon-autosuggest]', row).val($(this).data('label'));
      $('[data-taxon-autosuggest]', row).next().val($(this).data('taxon-id'));
      return false;
    });
  },
  initQuickAdd: function () {
    ...
    this.initTaxonSuggestField('#species-quick-add', function (input, item) {
      ...
      $('[data-taxon-autosuggest]', row).val(item.value);
      $('[data-taxon-autosuggest]', row).next().val(item.id);
      input.value = '';
      window.scrollTo(0, document.body.scrollHeight);
    });
  ...
  initTaxaAutosuggest: function () {
    return this.initTaxonSuggestField("[data-taxon-autosuggest]");
  },
  addNewRow: function () {
    var row_html;
    this.last_row_num++;
    ...
    this.initTaxonSuggestField(row.find('[data-taxon-autosuggest]')[0]);
    return $('.obs-row:last');
  },
