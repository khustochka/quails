...
namespace :tax do
  namespace :update2022 do
    ...
    task import_ebird_csv: :environment do
      filename = ENV["CSV"] || "./data/eBird_Taxonomy_v2022_conv.csv"
    ...
    task :list_rank_changes => :environment do
      taxa_rank = Taxon.joins("LEFT OUTER JOIN ebird_taxa ON taxa.ebird_code = ebird_taxa.ebird_code").includes(:ebird_taxon).where("taxa.category != ebird_taxa.category OR ebird_taxa.ebird_version != 2022")

      puts "RANK CHANGED:\n\n"

      data = taxa_rank.map {|tx| et = tx.ebird_taxon.ebird_version == 2022 ? tx.ebird_taxon : nil
    ...
    task clean_ebird_taxa: :environment do
      old_etaxa = EBirdTaxon.where(ebird_version: 2021)
      to_fix = Taxon.joins(:ebird_taxon).merge(old_etaxa)
      num_to_fix = to_fix.count
      puts "\n\nREPORT:"
      puts "Number of 2021 taxa left: #{old_etaxa.count}"
