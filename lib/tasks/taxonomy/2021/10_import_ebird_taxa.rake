...
namespace :tax do
  namespace :update2021 do
    ...
    task import_ebird_csv: :environment do
      filename = ENV["CSV"] || "./data/eBird_Taxonomy_v2021.csv"
    ...
    task :list_rank_changes => :environment do
      taxa_rank = Taxon.joins("LEFT OUTER JOIN ebird_taxa ON taxa.ebird_code = ebird_taxa.ebird_code").includes(:ebird_taxon).where("taxa.category != ebird_taxa.category OR ebird_taxa.ebird_version != 2021")

      puts "RANK CHANGED:\n\n"

      data = taxa_rank.map {|tx| et = tx.ebird_taxon.ebird_version == 2021 ? tx.ebird_taxon : nil
    ...
    task clean_ebird_taxa: :environment do
      ...
      puts "\n\nREPORT:"
      puts "Number of 2019 taxa left: #{old_etaxa.count}"
