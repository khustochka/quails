...
namespace :tax do
  namespace :update2021 do
    task list_taxa_changes: :environment do
      taxa_rank = Taxon.joins(:ebird_taxon).includes(:ebird_taxon).where("taxa.category != ebird_taxa.category OR ebird_taxa.ebird_version != 2021")

      # puts "RANK CHANGED:\n\n"
      #
      # taxa_rank.each do |tx|
      #   puts "#{tx.name_sci} (#{tx.category})"
      #   if tx.ebird_taxon.ebird_version == 2021
