...
namespace :tax do
  namespace :update2025 do
    ...
    task update_species: :environment do
      ...
      huds_whim = Taxon.find_by_ebird_code("whimbr3")
      huds_whim.species_id = nil
      huds_whim.save!
      huds_whim.lift_to_species

      euro_whim = EBirdTaxon.find_by_ebird_code("whimbr5").promote

      SpeciesSplit.create!(superspecies: euro_whim.species, subspecies: huds_whim.species)
      LocalSpecies.where(locus: Locus.find_by_slug("manitoba"), species: euro_whim.species).update!(species: huds_whim.species)

      # Yellow Warbler
      north_yewa = Taxon.find_by_ebird_code("yelwar1")
      # old_sp = north_yewa.species
      north_yewa.species_id = nil
      north_yewa.save!
      # new_sp = north_yewa.lift_to_species

      # UrlSynonym.create(name_sci: old_sp.name_sci, species: new_sp, reason: "split")
      # LocalSpecies.where(locus: Locus.find_by_slug("manitoba"), species: old_sp).update!(species: new_sp)
      ...
      spcs.each do |sp|
        ...
        if sp.high_level_taxa.empty?
          ...
          if ranks.include?("slash")
            case sp.name_sci
            when "***"
              # ebtx = EBirdTaxon.find_by_ebird_code("eawvir1")
            ...
            when "Setophaga petechia"
              taxon = Taxon.find_by_ebird_code("yelwar1")
              taxon.lift_to_species(species: sp)
              sp.high_level_taxa.reload
              synonym_type = "split"
              puts "*** #{sp.name_sci} split into #{sp.high_level_taxon.name_sci}."
            else
