...
namespace :tax do
  namespace :update2023 do
    ...
    task import_ebird_csv: :environment do
      filename = ENV["CSV"] || "./data/ebird_taxonomy_v2023.csv"
    ...
    task :list_rank_changes => :environment do
      taxa_rank = Taxon.joins("LEFT OUTER JOIN ebird_taxa ON taxa.ebird_code = ebird_taxa.ebird_code").includes(:ebird_taxon).where("taxa.category != ebird_taxa.category OR ebird_taxa.ebird_version != 2023")

      puts "RANK CHANGED:\n\n"

      data = taxa_rank.map {|tx| et = tx.ebird_taxon.ebird_version == 2023 ? tx.ebird_taxon : nil
    ...
    task :fix_removed_codes => :environment do
      # Am. Goshawk issf was removed, and new Am. Goshawk species have the code `norgos``
      old_am_goshawk = Taxon.find_by(ebird_code: "norgos2")
      new_am_goshawk = EBirdTaxon.find_by(ebird_code: "norgos").promote
      old_am_goshawk.observations.each {|obs| obs.update!(taxon: new_am_goshawk)}
      old_am_goshawk.destroy!
    ...
    task clean_ebird_taxa: :environment do
      old_etaxa = EBirdTaxon.where(ebird_version: 2022)
      to_fix = Taxon.joins(:ebird_taxon).merge(old_etaxa)
      num_to_fix = to_fix.count
      puts "\n\nREPORT:"
      puts "Number of 2022 taxa left: #{old_etaxa.count}"
