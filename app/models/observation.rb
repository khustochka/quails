class Observation < ActiveRecord::Base
  ...
  def self.lifers_observations(*args)
    ...
    select('DISTINCT ON (observations.species_id, observ_date) observations.*').joins("INNER JOIN (#{lifers_dates(options).to_sql}) AS dts
          ON observations.species_id=dts.species_id AND observations.observ_date=dts.first_date")
  end

  def self.lifelist(*args)
    lifers_observations(*args).includes(:species, :post).order('observ_date DESC')
  end
