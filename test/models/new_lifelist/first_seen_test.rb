...
class FirstSeenTest < ActiveSupport::TestCase
  ...
  test 'Species lifelist by count return proper number of species' do
    assert_equal @obs.map(&:species_id).uniq.size, NewLifelist::FirstSeen.full.sort('count').size
  end

  test 'Species lifelist by taxonomy return proper number of species' do
    assert_equal @obs.map(&:species_id).uniq.size, NewLifelist::FirstSeen.full.sort('class').size
  end
  ...
  test 'Species lifelist by taxonomy properly sorts the list' do
    ...
    actual = NewLifelist::FirstSeen.full.sort('class').map { |s| [s.species.code, s.observ_date.iso8601] }
    assert_equal expected, actual
  end

  test 'Species lifelist by date return proper number of species' do
    assert_equal @obs.map(&:species_id).uniq.size, NewLifelist::FirstSeen.full.size
  end
  ...
  test 'Species lifelist by date properly sorts the list' do
    ...
    actual = NewLifelist::FirstSeen.full.map { |s| [s.species.code, s.observ_date.iso8601] }
    assert_equal expected, actual
  end

  test 'Year list by taxonomy properly sorts the list' do
    expected = [["colliv", "2009-10-18"], ["parmaj", "2009-01-01"], ["pasdom", "2009-01-01"]]
    actual = NewLifelist::FirstSeen.over(year: 2009).sort('class').map { |s| [s.species.code, s.observ_date.iso8601] }
    assert_equal expected, actual
  end

  test 'List by super locus returns properly filtered list' do
    expected = [["colliv", "2010-03-10"], ["pasdom", "2009-12-01"]]
    actual = NewLifelist::FirstSeen.over(locus: 'usa').map { |s| [s.species.code, s.observ_date.iso8601] }
    assert_equal expected, actual
  end

  test 'List by locus and year returns properly filtered list' do
    expected = [["pasdom", "2010-04-16"], ["colliv", "2010-03-10"]]
    actual = NewLifelist::FirstSeen.over(locus: 'usa', year: 2010).map { |s| [s.species.code, s.observ_date.iso8601] }
    assert_equal expected, actual
  end
